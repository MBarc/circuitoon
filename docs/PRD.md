...
- **Broken references.** A connection whose endpoint no longer resolves (a missing part, pin or hole group, an out-of-range hole index, or a bus offset past the end) still loads and stays in the file: it never conducts, and its one resolvable end draws a short (20 px) dashed red stub, above wire labels, that the user can select and delete like any other wire. It is never silently dropped.
...
Not built yet: RGB LED, buzzer, slide switch, rocker switch, toggle switch, diode, NPN and PNP transistor, N-channel MOSFET, USB power, DC barrel jack, 3.3V and 5V regulators, Arduino Uno, Arduino Nano, Raspberry Pi Pico, pin header.
