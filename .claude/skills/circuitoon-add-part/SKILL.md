...
3. **Choose id, name and category.**
   - `id`: lowercase kebab-case, descriptive and stable (`tft-ili9341-28-spi-touch`). Designator prefixes key off the id (see `src/editor/ops.ts` PREFIXES; first match wins, in this order): `potentiometer*` gives RV, `resistor*` R, `capacitor*` C, `led*` D, `ws2812*` D, `*button*|*switch*` S, `battery*` BT, `lcd-|oled-|tft-` DS, `piezo*|buzzer*` BZ, `relay*` K, `servo*` M, `jst-|dupont-|usb-panel-` J, everything else U. Add a prefix rule (with a test) if a new family needs one.
...
5. **Draw the art** in the Sticker style (rectangles only; the renderer adds the ink outline). See `references/conventions.md`. For families of similar parts, extend or add a generator script in `scripts/` (existing: `gen-boards.mjs` for ESP32, Arduino Nano and D1 mini boards, `gen-picos.mjs` for the Raspberry Pi Picos, `gen-parts.mjs` for chips, displays and microSD modules, `gen-power.mjs` for power modules, `gen-sensors.mjs` for addressable LEDs and sensor modules, `gen-outputs.mjs` for relay, servo, motor driver, radio and level shifter modules, `gen-typewriter.mjs` for the Spirit Typewriter parts: tilt and tactile switches, the ESP32 screw terminal adapter, the panel pot, USB panel-mount cables, JST-XH headers and Dupont housings, `gen-breadboards.mjs` for breadboards and rail strips) so pin lists live in one readable table; the generator must reproduce the committed JSON byte for byte. Name it `scripts/gen-<family>.mjs` and write its files through `emit`/`log`/`finish` from `scripts/lib/gen-output.mjs` so it supports `--check` (part generators get that for free through `write` in `scripts/lib/parts.mjs`, which also holds the shared `r`, `slots`, `side` and `moduleJson`; use them rather than copying); `npm run check:gen` (scripts/check-gen.mjs) picks up every `gen-*.mjs` automatically and fails one that does not import that helper.
...
- `supply`: the voltage the pin actually sees ("3V3", "5V", "9V", "3.7V" for a Li-ion cell), never a symbolic rail name like "VBAT". A power input that accepts several rails lists every one its datasheet range covers with "/" ("3V3/5V"), which keeps a later wrong-voltage check from flagging valid hookups. An adjustable output is "ADJ" (user-set). See "Supply names" in `references/conventions.md`.
...
- Header pins drawn inside the body next to each pin, like silkscreen, are opt-in with `"art": { ..., "pinLabels": "inside" }`. As applied: every part whose pins are a named header, pads or terminals (boards, the ESP32 terminal adapter, DIP chips, display, storage, power, sensor, relay, motor driver, radio and level shifter modules, 3-pin ones such as the AMS1117, DHT22 module, HC-SR501 and the SG90 servo lead included), multi-lead components whose leads carry signal names (WS2812D 5 mm LED, bare DHT22) and the USB panel-mount cables. Not on two-lead parts (batteries, passives, LED, buzzer, push, rocker, tilt and 4-pin tactile switches), numbered connector housings (JST-XH, Dupont) or the pots (potentiometer, WH148 panel pot), which keep the default labels.
